...
class Event extends AData {
  ...
  Event.fromICalJson(
      Map<String, dynamic> json, String calendar, DateTime sync_date)
      : uid = json['UID'].toString(),
        title = clean_ics_text_fields(json['SUMMARY']) ??
            '<< missing event title >>',
        start =
            DateTime.parse(json['DTSTART'] ?? get_ics_tz_key(json, 'DTSTART'))
                .toLocal(),
        end = DateTime.parse(json['DTEND'] ?? get_ics_tz_key(json, 'DTEND'))
            .toLocal(),
