  ...
  Widget build(BuildContext context) {
    var consumer = Consumer<EventList>(builder: (context, events, child) {
      ...
      for (Event e in events.list) {
        ...
        var start_day =
            DateTime(tmp.start.year, tmp.start.month, tmp.start.day);
        var end_day = DateTime(tmp.end.year, tmp.end.month, tmp.end.day);
        if (!dates.contains(start_day)) dates.add(start_day);
        if (end_day != start_day && !dates.contains(end_day))
          dates.add(end_day);

        fitted_events.add(tmp);

        var ev_start =
            HourMinute(hour: tmp.start.hour, minute: tmp.start.minute);
        ...
        Padding(
            ...
            child: WeekView(
                ...
                dayBarStyleBuilder: (date) =>
                    DayBarStyle(dateFormatter: (int year, int month, int day) {
                      ...
                      var year_str = (year == config.event_year
                          ...
                          : DateFormat.yMd(
                              ...
                              .format(date));
                      var str =
                          '${DateFormat.EEEE(Localizations.localeOf(context).languageCode).format(date)} $year_str';
                      return str;
                    }),
                dayViewStyleBuilder: (date) => DayViewStyle(
                    hourRowHeight: view_height / (24 - min_time.hour)),
                events: fitted_events
                    .map((e) => get_wkview_event(context, e))
                    .toList(),
      ...
      var refresh_indicator =
          RefreshIndicator(onRefresh: events.refresh, child: wk);
...
FlutterWeekViewEvent get_wkview_event(context, Event calendar_event) {
  return FlutterWeekViewEvent(
      decoration: BoxDecoration(
          ...
          color: config.calendars[calendar_event.type]?['color']),
      ...
      onTap: () {
        show_event_popup(calendar_event, context);
      });
}
...
void show_event_popup(Event event, BuildContext context) {
  ...
  if (event.location != null && event.location != '') {
    buttons.add(DialogButton(
        ...
        onPressed: () async {
          var url = (RegExp(r"-?[0-9]{1,2}\.[0-9]{6}, ?-?[0-9]{1,2}\.[0-9]{6}")
                  .hasMatch(event.location!))
              ? Uri(scheme: 'geo', host: event.location).toString()
              : Uri(
                  scheme: 'geo',
                  host: '0,0',
                  queryParameters: {'q': event.location}).toString();
          launch_url(url);
        },
        child: Row(
  ...
  if (event.summary != null) alert_children.add(Text(event.summary!));
