...
class PDFDialog extends StatelessWidget {
  final String pdf;
  final String name;
  const PDFDialog({Key? key, required this.pdf, required this.name}) : super(key: key);
      ...
      child: Column(children:
          [
            Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              ...
              [
                Text(name),
                ElevatedButton(onPressed: () => launchUrl(Uri.parse(pdf), mode: LaunchMode.externalApplication), child: const Text('download'))
              ]),
            Expanded(
              child: const PDF(fitPolicy: FitPolicy.BOTH).cachedFromUrl(pdf,
                placeholder: (progress) => Center(child: Column(children: [Text('$progress %'), const CircularProgressIndicator.adaptive()])),
...
class ResultsDialog extends StatelessWidget {
  ...
  build(context) {
    ...
    for (var res in results.list) {
      children.add(Center(child: ElevatedButton(child: Text(res.name), onPressed: () => {
            showDialog(context: context, builder: (context) => PDFDialog(name: res.name, pdf: res.pdf))
      })));
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (competitor_or_startlist != null && competitor_or_startlist != '')
      buttons.add(IconButton(icon: const Icon(Icons.groups), onPressed: () async {
            showDialog(context: context, builder: (context) => PDFDialog(name: competition.name, pdf: competitor_or_startlist));
        ...
        IconButton(icon: const Icon(Icons.format_list_numbered), onPressed: () async {
          if (competition.results.list.length > 1) 
            showDialog(context: context,
              builder: (context) => ResultsDialog(results: competition.results));
          else
            showDialog(context: context,
              builder: (context) => PDFDialog(name: competition.name, pdf: competition.results.list.first.pdf));
        }));
    return Padding(padding: const EdgeInsets.all(2),
      child: ListTile(title: Text(competition.name),
      tileColor: Colors.white,
      subtitle: Row(children: buttons),
      shape: RoundedRectangleBorder(side: const BorderSide(), borderRadius: BorderRadius.circular(5)),
      dense: true,
      onTap: () => {
        if ((competitor_or_startlist == null || competitor_or_startlist == '')
